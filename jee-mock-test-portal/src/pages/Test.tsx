  ...
  useEffect(() => {
    ...
    const preventShortcuts = (e: KeyboardEvent) => {
      if (e.key === 'PrintScreen') {
        ...
        showViolationWarning('Screenshot attempt detected');
      ...
      if (e.key === 'F12' ||
        ...
        e.preventDefault();
        showViolationWarning('DevTools access attempt');
      }
      if ((e.ctrlKey || e.metaKey) && e.key === 'p') {
        e.preventDefault();
        showViolationWarning('Print attempt detected');
      }
      if ((e.ctrlKey || e.metaKey) && e.key === 's') {
        e.preventDefault();
        showViolationWarning('Save attempt detected');
      }
    };
    ...
    const handleFullscreenChange = () => {
      ...
      if (!document.fullscreenElement && testStarted && !testCompleted) {
        setFullscreenExitCount(prev => {
          ...
          if (newCount >= 3) {
            showViolationWarning('Fullscreen exited repeatedly. Test submitted automatically.');
            setTestCompleted(true);
            setShowResults(true);
          } else {
            showViolationWarning('Exited fullscreen mode');
          }
          return newCount;
        });
        if (!testCompleted) {
